...
通常のユーザー導線は、公開済みの PyPI package です。custom Homebrew tap は通常の install path にはしません。
...
release asset を目で確認しながら入れたい場合は、checksum 検証付きの GitHub Release installer も使えます。
...
source checkout で開発する場合は、compatibility wrapper を直接呼べます。
...
## 現状の結論: tap ではなく PyPI/uvx と Release installer に寄せる
...
3. このリポジトリは PyPI/uvx または release installer で入れる。
...
PyPI package の基本形は、次の `uvx` / `uv tool install` です。
...
source checkout で開発する場合は、repo 内の compatibility wrapper を直接呼べます。
